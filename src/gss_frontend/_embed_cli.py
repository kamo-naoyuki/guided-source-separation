...
def _load_segments(seglst_file: Path) -> Optional[List[Dict]]:
    """Load segments from a SegLST JSON file (.seglst or .json).
    ...
    """
    if seglst_file.suffix not in (".seglst", ".json"):
...
def _embed_speaker_segments(
    ...
    Args:
        ...
        sr: Sample rate
        segments: List of segment dicts with start_time, end_time, audio_path
    ...
    for seg in segments:
        ...
        # Undo the clipping-avoidance normalization applied by gss-enhance
        scale = seg.get("scale", 1.0)
        if scale != 1.0:
            enhanced_seg = enhanced_seg * scale
